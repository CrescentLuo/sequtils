...
def get_phastcons(interval, phastcons_bw, nBins=10):
    ...
    interval['phastcons_bin' + '(bins={})'.format(nBins)] = ','.join([str(s)
                                                                     for s in phastcons_bw.stats(chrom, start, end, nBins=nBins)])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Calculation of PhastCons of genes using bigwig file")
    parser.add_argument(
        '-p', '--phastcons', required=True,
        help='Phast conservation in bigwig format')
    parser.add_argument(
        '-b', '--bed', required=True,
        help='Interested gene genomic location in BED format')
    parser.add_argument(
        '-n', '--nBins', type=int, default=10,
        help='number of bins for phastcons calculation')
